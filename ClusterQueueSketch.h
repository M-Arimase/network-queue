...
#ifndef _NS_QCLUSTER_QUEUE_SKETCH_H_
#define _NS_QCLUSTER_QUEUE_SKETCH_H_
...
class ClusterQueueSketch : public Queue{
public:
	ClusterQueueSketch();
	~ClusterQueueSketch();
protected:
...
ClusterQueueSketch::ClusterQueueSketch()
{
	...
	tmsketch = new TimeSketch(interval_msg_, interval_qlet_, queue_num_);
...
void ClusterQueueSketch::Update_Queue(int q)
{
	info[q].counting /= 2.0;
	info[q].distinct /= 2.0;
	return;
...
void ClusterQueueSketch::Update()
{
...
//Return total queue length (bytes) of all the queues
int ClusterQueueSketch::TotalByteLength()
{
	int bytelength = 0;
	for(int i = 0; i < MAX_QUEUE_NUM; ++i)
